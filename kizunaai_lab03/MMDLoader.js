...
THREE.MMDLoader = ( function () {
	...
	GeometryBuilder.prototype = {
		...
		build: function ( data ) {
			...
			// groups
			data.textures = data.textures.map(e => e.replace(/\\/g, '/'))
		...
		build: function ( data, geometry /*, onProgress, onError */ ) {
			...
			for ( var i = 0; i < data.metadata.materialCount; i ++ ) {
				...
				if ( data.metadata.format === 'pmd' ) {
					...
					if(!/Face 1/i.test(params.name)){
						params.emissive.r = 0
						params.emissive.g = 0
						params.emissive.b = 0
					}
						
					if(/shoe/i.test(params.name)){
						params.color.r = 0.34901960784313724
						params.color.g = 0.34901960784313724
						params.color.b = 0.34901960784313724
					}
				}
